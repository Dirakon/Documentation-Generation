...
def post_process_code_to_eo_standards(eo_code: str)->str:
    return '\n'.join(
        [discard_comments(line).rstrip() for line in eo_code.split('\n')
            if len(discard_comments(line.strip())) > 0
            ])

def discard_comments(line: str)->str:
    line_split_by_comment_mark = line.split("#")
    if len(line_split_by_comment_mark) != 2:
        return line
    return line_split_by_comment_mark[0]
